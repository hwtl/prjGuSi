//社保管理--社会保险--宁波新进列表页控制器
function SiNbNewListCtrl($scope, $http, $rootScope, $filter, $route){
/*************** 初始化 ****************/
	$scope.init = {
...
/*************** 事件定义***************/
	...
	$scope.getBatchInfo = function(){
		if($scope.query.paymentDate){
			...
			loadData($http, "/si/nbnew/switchdate", $scope, 'batch', {params: data, callback: function(){//
				var t = new Date();
				...
				if(today== paymentDate){//当前月
					if($scope.batch.batchInfo){//有批次
						...
						if($scope.query.batchId){
							$scope.exportExcel = "/si/nbnew/export?batchId=" + $scope.query.batchId;
						}
						//如果是页面初始化，则查询
	...
	$scope.search = function(isBtnTrigger){
		...
		loadData($http, "/si/nbnew/list", $scope, 'result', {params:data, callback:function(){
	...
	$scope.createBatch = function(){
		loadData($http, "/si/nbnew/gbbegintime", $scope, 'newBatch', {callback:function(){
			var beginTime = $filter('date')($scope.newBatch.batchBeginTime, 'yyyy-MM-dd HH:mm:ss');
			if(window.confirm("确认拉 " + beginTime + "至今的宁波缴纳名单？")){
				showPopupDiv($("#layer_ajaxing"));
				var data = 'paymentDate=' + $scope.query.paymentDate;
				$http({method:'POST', url:'/si/nbnew/generatebatch', data:data, headers : {
	...
	$scope.confirmNew = function(){
		...
		$http({method:'POST', url:'/si/nbnew/confirmpayment', data:data, headers : {
        'Content-Type' : 'application/x-www-form-urlencoded'
        }}).success(function(data, status, headers, config){
            if(data.status == "ok"){
	...
	$scope.editSave = function(){
		...
		if(validator.validateForm()){
			...
			//发送请求
			$http({method:'POST', url:'/si/nbnew/edit' , data:data, headers : {
            'Content-Type' : 'application/x-www-form-urlencoded'
            }}).success(function(data, status, headers, config){
...
/*************** 数据处理 **************/
	...
	function dataProcess_edit(){
		...
		data += "id=" + $scope.params.id + //记录id
			    "&paymentBase=" + $scope.params.paymentBase + //社保基数
			    "&requireDate=" + $filter('date')($scope.params.requireDate, 'yyyy-MM-dd') + //要求参保起始月份
