 ...
 */
@Controller
@RequestMapping("/si/szquit/**")
public class SZQuitController extends BomsBaseController {

    private static final Log LOG = LogFactory.getLog(SZQuitController.class);
    ...
    @ResponseBody
    public JsonResult list(BaseSearch szSearch, HttpServletRequest request) {
        try {
            Employee employee = this.getSesionUser(request);
            if(!employee.checkPrivilege(Constants.HRM_SI_SZ_VIEW)) {
                return fail("没有权限！");
            }
            Map<String, Object> data = new HashMap<>();
            data.put("resultList", siBatchService.queryForPaginateSZQuit(szSearch));
            data.put("totalCount", siBatchService.countForPaginateSZQuit(szSearch));
            data.put("pageNo", szSearch.getPageNo());
            data.put("pageSize", szSearch.getPageSize());
            return ok().put("data", data);
        } catch (Exception e) {
            LOG.error("查询失败。" + szSearch, e);
            return fail();
        }
    }
    ...
    public JsonResult switchDate(@RequestParam Date paymentDate) {
        try {
            Map<String, Object> data = new HashMap<>();
            data.put("batchInfo", siBatchInfoService.queryForListSZQuit(paymentDate));
            return ok().put("data", data);
        } catch (Exception e) {
    ...
    public JsonResult confirmQuit(@RequestParam String batchId,HttpServletRequest request) {
        try {
            Employee employee = this.getSesionUser(request);
            if(!employee.checkPrivilege(Constants.HRM_SI_SZ_OPERATE)) {
                return fail("没有权限！");
            }
    ...
    public JsonResult getBatchBeginTime(HttpServletRequest request) {
        try {
            ...
            Map<String, Object> data = new HashMap<>();
            data.put("batchBeginTime", siBatchInfoService.getPreviousBatchDateSZQuit());
            return  ok().put("data", data);
        } catch (Exception e) {
            LOG.error("拉名单失败。", e);
            return fail();
    ...
    public JsonResult generateBatch(@RequestParam Date paymentDate, HttpServletRequest request) {
        try {
            ...
            return siBatchInfoService.generateSZQuit(employee.getUserCode(), paymentDate) ? ok() : fail("拉名单失败。");
        } catch (Exception e) {
            LOG.error("拉名单失败。", e);
            return fail();
        }
    ...
    public void export(BaseSearch baseSearch, HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            siBatchService.exportExcelSZQuit(response, baseSearch);
        } catch (Exception e) {
