...
public class ExportExcelUtil<T> {
    ...
    public  void writeExcel(List<T> stream, OutputStream out, String sheetName,String[] headers, String[] execlKey ) throws Exception {
        ...
        if (StringUtils.isBlank(sheetName))
            sheetName = SHEET_NAME + DateFormatUtil.getCurrent("yyyyMMddHHmm");
        ...
        for(int i = 0; i<stream.size();i++){
            ...
            for (int j = 0; j < execlKey.length; j++) {
                if (null != execlKey[j] && StringUtils.isNotBlank(execlKey[j])) {
                    cell = row.createCell(j);
                    cell.setCellValue(ReflectionUtil.getFieldValueByName(execlKey[j],stream.get(i),DateFormatUtil.YMD_FORMAT).toString());
                }
            }
        }
        try {
        ...
        while (it.hasNext()) {
            ...
            for (short i = 0; i < fields.length; i++) {
                ...
                try {
                    ...
                    if (textValue != null) {
                        if(textValue.contains("http")){ //判断是否是超链接的存在
